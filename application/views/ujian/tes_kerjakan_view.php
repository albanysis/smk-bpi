...
<script type="text/javascript">
    ...
    function ragu() {
        ...
        $.ajax({
            url: '<?php echo base_url() . '/' . $url; ?>/get_tes_soal_by_tessoal/' + $('#tes-soal-id').val(),
            type: "POST",
            cache: false,
            timeout: 10000,
            success: function(respon) {
                ...
                if (data.data == 1) {
                    ...
                    $.ajax({
                        url: '<?php echo base_url() . '/' . $url; ?>/update_tes_soal_ragu/' + $('#tes-soal-id').val() + '/' + ragu,
                        type: "POST",
                        cache: false,
                        timeout: 5000,
                        success: function(respon) {
    ...
    function soal(tessoal_id) {
        $("#modal-proses").modal('show');
        $.ajax({
            url: '<?php echo base_url() . '/' . $url; ?>/get_soal_by_tessoal/' + tessoal_id + '/' + $('#tes-user-id').val(),
            type: "POST",
            cache: false,
            timeout: 10000,
            success: function(respon) {
    ...
    function audio(status) {
        ...
        if (status == 1) {
            if (audio_player_update == 0) {
                 ...
                 */
                $.getJSON('<?php echo base_url() . '/' . $url; ?>/update_status_audio/' + $('#tes-soal-id').val(), function(data) {
                    if (data.data == 1) {
    ...
    function hentikan_tes() {
        ...
        $.getJSON('<?php echo base_url() . '/' . $url; ?>/get_tes_info/' + $('#tes-id').val(), function(data) {
            if (data.data == 1) {
    ...
    $(function() {
        ...
        $('#form-kerjakan').submit(function() {
            $("#modal-proses").modal('show');
            $.ajax({
                url: "<?php echo base_url() . '/' . $url; ?>/simpan_jawaban",
                type: "POST",
        ...
        $('#form-hentikan').submit(function() {
            $("#modal-proses").modal('show');
            $.ajax({
                url: "<?php echo base_url() . '/' . $url; ?>/hentikan_tes",
                type: "POST",
